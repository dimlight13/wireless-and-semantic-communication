    ...
    root.innerHTML = `
      ...
      <div class="vis-panel eq-demo">
        <section>
          ...
          <p class="hint">The radio channel adds complex gain H (amplitude attenuation plus phase rotation) and noise to QAM symbols. The receiver estimates H and applies Y/H to recover points near the original constellation.</p>
        </section>
        ...
        <section class="eq-card">
          <div class="eq-controls">
            ...
            <label>Channel magnitude |H|: <b>${state.channelMag.toFixed(2)}</b>
              <input type="range" min="0.2" max="1.5" step="0.05" value="${state.channelMag}" data-control="channelMag" />
            </label>
            <label>Channel phase ∠H: <b>${state.channelPhase}°</b>
              <input type="range" min="-180" max="180" step="5" value="${state.channelPhase}" data-control="channelPhase" />
            </label>
            <label>Noise σ: <b>${state.noiseSigma.toFixed(2)}</b>
              <input type="range" min="0" max="0.3" step="0.01" value="${state.noiseSigma}" data-control="noiseSigma" />
            </label>
            <button class="vis-button" data-action="noise" type="button" style="align-self:end">New Noise Seed</button>
          </div>

          <div class="eq-legend">
            <span><i style="background:#94a4ba;border:1px dashed #94a4ba"></i>Ideal Tx constellation</span>
            <span><i style="background:#245fd6"></i>Tx</span>
            <span><i style="background:#d94242"></i>Rx Y = H·X + n</span>
            <span><i style="background:#11865a"></i>After equalization X̂ = Y/Ĥ</span>
          </div>

          <div class="eq-plots">
            ${plot(refs, "#245fd6", refs, "Tx constellation X")}
            ${plot(received, "#d94242", refs, "Rx (distortion + noise)")}
            ${plot(equalized, "#11865a", refs, "After equalization X̂")}
          </div>

          <div class="eq-metrics">
            <span>Estimated channel Ĥ: ${state.channelMag.toFixed(2)} ∠ ${state.channelPhase}°</span>
            <span class="bad">Rx MSE: ${mseRx.toFixed(3)}</span>
            <span class="good">MSE after equalization: ${mseEq.toFixed(3)}</span>
            <span>MSE improvement: <b>${((1 - mseEq / Math.max(mseRx, 1e-6)) * 100).toFixed(1)}%</b></span>
          </div>
        </section>
      </div>
    `;
