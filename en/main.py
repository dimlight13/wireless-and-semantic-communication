...
<body>
  <div class="wrap">
    <header>
      ...
      <p>An interactive playground for comparing and learning two communication approaches. Choose a track to begin.</p>
    </header>

    <section class="cards">
      <a class="card trad" href="/traditional">
        <span class="tag">TRADITIONAL · 5G NR</span>
        <h2>Traditional Communication: 5G NR Uplink Protocol Stack</h2>
        <p>Explore the complete O-RU to O-DU to O-CU uplink pipeline block by block, including PHY, MAC, PDCP, and SDAP behavior.</p>
        <ul>
          <li>RF sampling · CP removal · OFDM FFT · digital beamforming</li>
          <li>Channel estimation/equalization · QAM demodulation · LDPC decoding · HARQ</li>
          <li>PDCP reordering/deciphering · SDAP QoS mapping</li>
        </ul>
      ...
      <a class="card sem" href="/semcom">
        ...
        <h2>Semantic Communication: Semantic Communication Explorer</h2>
        <p>A next-generation hybrid communication model that sends meaning instead of raw bits. Try the sLLM encoder, VQ, World Model, and KB-based decoder directly.</p>
        <ul>
          <li>sLLM Encoder · Vector Quantization · World Model</li>
          <li>Reuse of the existing RAN pipeline (hybrid stack)</li>
          <li>KB decoder · Bit-Com vs Sem-Com benchmark</li>
        </ul>
    ...
    <footer>
      Run: <code>python main.py</code> · URL: __ORIGIN__
    </footer>
  </div>
</body>
</html>
  ...
  <script type="module">
    ...
    try {{
      ...
      if (typeof module.mount !== "function") {{
        throw new Error("{entry_script} does not export a mount(root) function.");
      }}
      ...
      err.className = "error";
      err.innerHTML = `Execution failed<br>${{error.message}}`;
...
TRADITIONAL_HTML = _app_html(
    ...
    accent="#245fd6",
    loading_msg="Loading the protocol stack overview.",
)
...
SEMCOM_HTML = _app_html(
    ...
    accent="#6b43bd",
    loading_msg="Loading the Semantic Communication Explorer.",
)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"  Home (Track Select):  {url}/")
    print(f"  Traditional Communication:        {url}/traditional")
    print(f"  Semantic Communication:      {url}/semcom")
    print("=" * 60)
    try:
