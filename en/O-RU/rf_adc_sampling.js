export const metadata = {
  title: "RF Reception and ADC Sampling",
};
  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      <div class="vis-panel rf-demo">
        <section>
          <h2>RF Reception and ADC Sampling</h2>
          <p class="hint">Convert the analog RF waveform entering the antenna into digital I/Q samples according to the sampling rate and ADC bit depth. Adjust the sliders to inspect sample spacing and quantization steps.</p>
        </section>

        <section class="rf-card">
          <div class="vis-row">
            <button class="vis-button primary" data-action="toggle" type="button">${state.running ? "⏸ Pause Waveform" : "▶ Run Waveform"}</button>
            <button class="vis-button" data-action="step" type="button">+ One Step</button>
          </div>
          <div class="rf-controls">
            <label>Sampling rate (samples/cycle): <b>${state.sampleRate}</b>
              <input type="range" min="4" max="64" step="1" value="${state.sampleRate}" data-control="sampleRate" />
            </label>
            <label>ADC bit depth: <b>${state.bitDepth}</b> bit (${Math.pow(2, state.bitDepth)} levels)
              <input type="range" min="1" max="6" step="1" value="${state.bitDepth}" data-control="bitDepth" />
            </label>
            <label>Signal frequency: <b>${state.freq}</b> cycle
              <input type="range" min="1" max="6" step="1" value="${state.freq}" data-control="freq" />
            </label>
          </div>
          <div class="rf-legend">
            <span><i style="background:#245fd6"></i>Analog RF</span>
            <span><i style="background:#c86414"></i>Sampling points</span>
            <span><i style="background:#11865a"></i>Quantized digital values</span>
            <span><i style="background:#c7d3e3;border-radius:2px"></i>Quantization level grid</span>
          </div>
          ...
          <svg class="rf-svg" viewBox="0 0 ${WIDTH} ${HEIGHT}" preserveAspectRatio="xMidYMid meet">
            ...
            <text x="${PAD_L + PLOT_W / 2}" y="${HEIGHT - 8}" text-anchor="middle" font-size="11" fill="#35475d" font-weight="900">Time (1 cycle)</text>
          </svg>

          <div class="rf-metric">
            <span>Sample count: ${state.sampleRate}</span>
            <span>Quantization levels: ${Math.pow(2, state.bitDepth)}</span>
            <span>LSB size: ${(2 / Math.pow(2, state.bitDepth)).toFixed(3)}</span>
            <span>Nyquist condition: ${state.sampleRate >= 2 * state.freq ? "Satisfied ✓" : "Not met (aliasing risk)"}</span>
          </div>

          <div class="rf-iq">[I/Q Sample Output]\n${iqList.map((s) => `  sample[${String(s.idx).padStart(2, "0")}] = I: ${s.i}, Q: ${s.q}`).join("\n")}</div>
        </section>
      </div>
    `;
