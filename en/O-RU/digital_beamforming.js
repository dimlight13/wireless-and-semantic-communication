  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      <div class="vis-panel bf-demo">
        <section>
          <h2>Digital Beamforming: Antenna Array Weights</h2>
          <p class="hint">Multiplying each antenna's received signal by complex weights w<sub>n</sub> and combining them strengthens signals from a target angle while suppressing interference from other directions.</p>
        </section>

        <section class="bf-card">
          <div class="bf-controls">
            <label>Antenna count N: <b>${state.numAntennas}</b>
              <input type="range" min="2" max="16" step="1" value="${state.numAntennas}" data-control="numAntennas" />
            </label>
            <label>Beam steering angle: <b>${state.steeringAngle}°</b>
              <input type="range" min="-80" max="80" step="1" value="${state.steeringAngle}" data-control="steeringAngle" />
            </label>
            <label>Target UE angle: <b>${state.targetAngle}°</b>
              <input type="range" min="-80" max="80" step="1" value="${state.targetAngle}" data-control="targetAngle" />
            </label>
            <label>Interferer angle: <b>${state.interfererAngle}°</b>
            ...
            <button class="vis-button primary" data-action="align" type="button" style="align-self:end">Align steering to target</button>
          </div>

          <h3 style="margin-top:14px">Beam Pattern (Array Factor)</h3>
          <svg class="bf-svg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
            ...
            ${state.interfererOn ? angleLine(state.interfererAngle, "#d94242", `Interference (${state.interfererAngle}°)`, true) : ""}
            ${angleLine(state.steeringAngle, "#c86414", `Steering (${state.steeringAngle}°)`)}
            ...
            <text x="${CX}" y="${CY + 28}" text-anchor="middle" fill="#35475d" font-size="11" font-weight="900">Antenna array (N=${state.numAntennas}, spacing λ/2)</text>
          </svg>

          <div class="bf-metrics">
            <span class="ok">Target-direction gain: ${(targetGain * 100).toFixed(1)}% (${(20 * Math.log10(Math.max(0.001, targetGain))).toFixed(1)} dB)</span>
            <span class="${state.interfererOn && interfGain > 0.1 ? "bad" : ""}">Interference-direction gain: ${(interfGain * 100).toFixed(1)}% (${state.interfererOn ? (20 * Math.log10(Math.max(0.001, interfGain))).toFixed(1) + " dB" : "N/A"})</span>
            <span class="${sinr >= 10 ? "ok" : "bad"}">SINR: ${sinr > 90 ? "∞" : sinr.toFixed(1) + " dB"}</span>
          </div>

          <h3 style="margin-top:14px">Per-antenna weights w<sub>n</sub></h3>
