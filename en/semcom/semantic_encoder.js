export const metadata = {
  title: "sLLM Encoder: Meaning Extraction",
};

export function mount(root) {
  const presets = [
    "I plan to eat pizza with friends near Gangnam Station tonight",
    "A ripe red apple is in a basket on the dining table",
    "The Seoul Line 2 northbound train will arrive in 10 minutes",
    "Due to heavy rain, tomorrow morning's meeting will be held online",
  ];

  const stopwords = new Set(["a", "an", "the", "is", "are", "to", "with", "near", "on", "in", "of", "and", "will", "be", "due"]);

  const importanceMap = {
    "tonight": 0.9, "Gangnam": 0.95, "Station": 0.9, "friends": 0.8, "pizza": 0.95, "eat": 0.85,
    "red": 0.7, "ripe": 0.65, "apple": 0.95, "basket": 0.7, "dining": 0.6, "table": 0.6,
    "Seoul": 0.75, "Line": 0.8, "2": 0.8, "northbound": 0.85, "train": 0.9, "10": 0.85, "minutes": 0.8, "arrive": 0.8,
    "heavy rain": 0.95, "tomorrow": 0.9, "morning": 0.75, "meeting": 0.9, "online": 0.95,
    "near": 0.4, "plan": 0.5, "held": 0.5, "because": 0.3, "online": 0.95,
  };
  ...
  function render() {
    ...
    function embedSvg() {
      ...
      return `
        <svg viewBox="0 0 ${embedSize} ${embedSize}" class="se-embed">
          ...
          <text x="${CX}" y="14" text-anchor="middle" font-size="10" fill="#607083" font-weight="900">Semantic Embedding Space (2D Projection)</text>
        </svg>
      `;
    }
    ...
    root.innerHTML = `
      ...
      <div class="vis-panel se-demo">
        <section>
          <h2>sLLM Encoder: Extract Meaning from a Sentence</h2>
          <p class="hint">A small LLM discards filler and common words, keeping only the key contextual tokens. The compressed token count becomes the amount of meaning to transmit.</p>
        </section>

        <section class="se-card">
          <h3>1. Input Message</h3>
          ...
          <div class="se-controls">
            <label>Top-K kept tokens: <b>${state.topK}</b>
        ...
        <section class="se-card">
          <h3>2. Token Importance (estimated sLLM attention)</h3>
          <p class="sub">Only tokens selected by Top-K are transmitted. The receiver-side KB fills the rest from context.</p>
          <div class="se-tokens">
            ${tokens.map((t) => `
        ...
        <section class="se-card">
          <h3>3. Transmitted Token Sequence</h3>
          <div class="se-top-row">
            ${topTokens.length === 0 ? `<small style="color:#607083;font-weight:900">No important tokens found. Enter a longer sentence.</small>` : topTokens.map((t) => `<span class="se-top-chip">${t.core}</span>`).join("")}
          </div>

          <div class="se-split">
            ${embedSvg()}
            <div>
              <h3>4. Compression Efficiency</h3>
              <div class="se-metrics">
                <div class="se-metric">
                  <small>Traditional communication (UTF-8)</small>
                  <b>${traditionalBits} bits</b>
                  <small>${utf8Bytes} bytes · bit-level</small>
                </div>
                <div class="se-metric good">
                  <small>Semantic (Top-K tokens)</small>
                  <b>${tokenBits} bits</b>
              ...
              <p style="margin-top:10px; font-size:12px; color:#4f6073; font-weight:800; line-height:1.4">
                → Selected tokens are converted into codebook indices in the next <b>Vector Quantization</b> step.
              </p>
            </div>
          </div>
        </section>
